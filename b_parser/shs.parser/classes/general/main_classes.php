...
if (!defined("LOG_FILENAME")) {
    define("LOG_FILENAME", filter_input(INPUT_SERVER, "DOCUMENT_ROOT") . "/bitrix/log.txt");
}

//AddMessage2Log(print_r(array('$arFields' => $arFields), true));
...
class SotbitContentParser {
    ...
    private function setContentIblock($arContent = array(), $iblock_id = false, $section_id = false, $detail_dom = "", $encoding = "utf-8") {
        ...
        foreach ($arContent['item'] as $i => $item) {
            ...
            $ci++;
            if ($isElement && !self::TEST && $shs_DEMO == 1)
                continue;
            $first = true;
            ...
            if ($this->preview_first_img == "Y")
                $PREVIEW_IMG = $this->parserFirstImg(phpQuery::newDocument($item['description']), "text/html;charset=" . LANG_CHARSET);
            $shs_preview = false;
            if ($this->detail_first_img == "Y")
                $DETAIL_IMG = $this->parserFirstImg(phpQuery::newDocument($DETAIL_TEXT), "text/html;charset=" . LANG_CHARSET);
            ...
            if ($this->code_element == "Y")
                $code = $arProperty["CODE"] = CUtil::translit($item['title'], LANGUAGE_ID, array(
                            "max_len" => 100,
            ...
            elseif ($this->date_active == "PUBLIC" && $unix)
            ...
            } elseif ($this->bool_preview_delete_tag == "Y")
    ...
    public function GetAuthForm($check = false) {
        if (isset($this->settings[$this->type]["auth"]["active"]) && $this->settings[$this->type]["auth"]["active"] != "Y")
            return false;
        elseif (!isset($this->settings[$this->type]["auth"]["active"]))
            return false;
        ...
        if ($check)
            if (isset($this->errors) && count(isset($this->errors)) > 0) {
                foreach ($this->errors as $error) {
                    if (isset($_POST["auth"]))
                        CAdminMessage::ShowMessage($error);
                }
        ...
        if ($check)
            if (isset($this->success) && count(isset($this->success)) > 0) {
    ...
    protected function CheckPageNavigationLess($n) {
        ...
        if ($this->arPageNavigationDelta[1]) {
            if ($n <= $this->arPageNavigationDelta[1])
                return $n;
        }elseif (!$this->arPageNavigationDelta[1])
            return true;
        return false;
    }
    ...
    protected function CheckOnePageNavigation() {
        if ($this->settings["catalog"]["pagenavigation_begin"] == 1 && $this->settings["catalog"]["pagenavigation_end"] == 1) {
            return true;
        } elseif (!$this->settings["catalog"]["pagenavigation_selector"])
            return true;
    ...
    protected function GetSortFields() {
        ...
        if ($this->isUpdate) {
            foreach ($this->isUpdate as $id => $val) {
                ...
                elseif ($id == "detail_img")
                    $this->arSortUpdate[] = "DETAIL_PICTURE";
                elseif ($id == "preview_descr")
                    $this->arSortUpdate[] = "PREVIEW_TEXT";
                elseif ($id == "detail_descr")
    ...
    protected function parseCatalog() {
        ...
        foreach ($this->arUrlSave as $rss):
            ...
            if ((!$this->CheckOnePageNavigation() && $this->agent) || (!$this->CheckOnePageNavigation() && !$this->agent && $this->settings["catalog"]["mode"] == "debug"))
    ...
    protected function IsEndSectionUrl() {
        if (!isset($this->settings["catalog"]["url_dop"]) || empty($this->settings["catalog"]["url_dop"]) || empty($this->arUrl))
            return true;
        $count = 0;
    ...
    protected function parseCatalogProducts() {
        ...
        foreach ($this->html[$element] as $el) {
            $ci++;
            if ($this->StepContinue($ci, $count))
                continue;
            if ($i == self::DEFAULT_DEBUG_ITEM && $this->settings["catalog"]["mode"] == "debug")
                break;
    ...
    protected function StepContinue($n, $count = 0) {
        ...
        if ($currentElement > 0 && $n <= $currentElement && $currentElement % $step == 0)
            return true;
        else
            return false;
    }
    ...
    protected function getMeasureByProductId($product_id = 0) {
        if (CModule::IncludeModule("catalog")) {
            $arMeasureRatio = array();
            $db = CCatalogMeasureRatio::getList(array(), array("PRODUCT_ID" => intval($product_id) ?: $this->elementID));
            $arItem = $db->Fetch();
            if ($arItem) {
                $arMeasureRatio["MEASURE_RATIO"] = $arItem["RATIO"];
                $arMeasureRatio["MEASURE_RATIO_ID"] = $arItem["ID"];
            }
        }
        return is_array($arMeasureRatio) ? $arMeasureRatio : false;
    }
    
    protected function getListCatalogMeasure() {
        if (CModule::IncludeModule("catalog")) {
            $arMeasure = array();
        ...
        return is_array($arMeasure) ? $arMeasure : false;
    }

    protected function changeMeasureProduct() {
        if (!empty($this->arFields["PROPERTY_VALUES"]["UNIT_MEASURE"])) {
            $arMeasureRatio = $this->getListCatalogMeasure();
            $unit_measure = strtolower($this->arFields["PROPERTY_VALUES"]["UNIT_MEASURE"]);
            foreach ($arMeasureRatio as $arItem) {
                switch ($unit_measure) {
                    case strtolower($arItem["MEASURE_TITLE"]):
                    case strtolower($arItem["SYMBOL_RUS"]):
                    case strtolower($arItem["SYMBOL_INTL"]):
                        $this->arProduct["MEASURE"] = $arItem["ID"];
                        break;
                    case "м²":
                        $this->arProduct["MEASURE"] = "7";
                        break;
                }
                //AddMessage2Log(print_r(array('$this->arProduct["MEASURE"]' => $this->arProduct["MEASURE"]), true));    
            }
        }
    }
    ...
    protected function AddProductCatalog() {
        ...
        //AddMessage2Log(print_r(array('$this->arProduct' => $this->arProduct, '$this->arFields' => $this->arFields), true));
    }
    ...
    protected function AddPriceCatalog() {
        ...
        if (!$this->arPrice || !$this->arPrice["PRICE"])
            return false;
        $isElement = $this->elementUpdate;
		...
		if (!empty($this->arPrice["PRICE"])) {
			$this->arPrice["PRICE"] = rtrim(str_replace("..", "", $this->arPrice["PRICE"]), ".");
			$arUnitPrice = explode(".", $this->arPrice["PRICE"]);
			if (count($arUnitPrice) > 2) {
				$format_price = "";
				foreach ($arUnitPrice as $u_cell => $u_price) {
					if (($u_cell + 1) === count($arUnitPrice)) {
						$format_price .= ".";
					}
					$format_price .= $u_price;
				}
				$this->arPrice["PRICE"] = $format_price;
			}
			$this->arPrice["PRICE"] = ceil($this->arPrice["PRICE"]);
		}
		
		//AddMessage2Log(print_r(array('$this->arPrice' => $this->arPrice), true));
        ...
        if ($arProp["PROPERTY_TYPE"] == "S") {
            if ($arProp["USER_TYPE"] !== "directory") {
                ...
                if ($arProp["MULTIPLE"] !== "Y") {
                    $this->arFields["PROPERTY_VALUES"][$code] = $this->CheckPropsD($arProp, $code, trim($val));
                } else {
                    $this->arFields["PROPERTY_VALUES"][$code]["n0"] = $this->CheckPropsD($arProp, $code, trim($val));
                }
            }
        } elseif ($arProp["PROPERTY_TYPE"] == "N") {
            if (strpos($val, "-")) {
                $arVal = explode("-", $val);
                $val = $arVal[1]; // берем максимальное значение, $arVal[0] - минимальное значение
            }
            $val = str_replace(",", ".", $val);
    ...
    protected function parseCatalogDefaultProp($code, $val) {
        ...
        if ($arProp["PROPERTY_TYPE"] == "S") {
            if ($arProp["USER_TYPE"] !== "directory") { // type string
                $this->arFields["PROPERTY_VALUES"][$code] = $val;
            } else { // type directory (справочник)
                if ($arProp["MULTIPLE"] !== "Y") {
                    $this->arFields["PROPERTY_VALUES"][$code] = $this->CheckPropsD($arProp, $code, trim($val));
                } else {
                    $this->arFields["PROPERTY_VALUES"][$code]["n0"] = $this->CheckPropsD($arProp, $code, trim($val));
    ...
    protected function CheckPropsD($arProp, $code, $val) {
        $TABLE_NAME = $arProp["USER_TYPE_SETTINGS"]["TABLE_NAME"]; // таблица БД справочника
        global $DB;
        $strSql = "SELECT * FROM {$TABLE_NAME} WHERE UF_NAME LIKE '{$DB->ForSql($val)}'";
        $rdb = $DB->Query($strSql, false, __LINE__);
        $arRow = $rdb->Fetch();
        if (!empty($arRow)) {
            ...
            $xml_id = "p_" . time();
            $arFields = array(
                "UF_NAME" => "'{$DB->ForSql($val)}'",
                "UF_SORT" => "'500'",
                "UF_XML_ID" => "'{$xml_id}'"
            );
            $id = $DB->Insert($TABLE_NAME, $arFields, __LINE__);
            $xml_id = $id ? $xml_id : false;
        }
        return $xml_id;
    }
    ...
    protected function CheckPropsL($id, $code, $val) {
        $res2 = CIBlockProperty::GetPropertyEnum(
            $id, array(), array("IBLOCK_ID" => $this->iblock_id, "VALUE" => $val)
        );
    ...
    protected function parseCatalogDetailMorePhoto(&$el) {
        if ($this->settings["catalog"]["more_image_props"]) {
            if ($this->checkUniq() && (!$this->isUpdate || !$this->isUpdate["more_img"]))
                return false;
            $code = $this->settings["catalog"]["more_image_props"];
    ...
    protected function parseCatalogDetailPicture(&$el) {
        ...
        if ($this->settings["catalog"]["detail_picture"]) {
            ...
            foreach ($arSelPic as $sel) {
                ...
                //AddMessage2Log(print_r(array('$src' => $src), true));
                $this->arFields["DETAIL_PICTURE"] = CFile::MakeFileArray($src);
                if (!self::CheckImage($this->arFields["DETAIL_PICTURE"]["name"], true)) {
                    $exp = explode("/", $this->arFields["DETAIL_PICTURE"]["type"])[1];
                    $tmp_name = $this->arFields["DETAIL_PICTURE"]["tmp_name"] . "." . $exp;
                    if (rename($this->arFields["DETAIL_PICTURE"]["tmp_name"], $tmp_name)) {
                        $this->arFields["DETAIL_PICTURE"]["name"] .= ".{$exp}";
                        $this->arFields["DETAIL_PICTURE"]["tmp_name"] = $tmp_name;
                    }
                }
                
                //AddMessage2Log(print_r(array('DETAIL_PICTURE' => $this->arFields["DETAIL_PICTURE"]), true));
    ...
    protected function CheckImage($src, $strict = false) {
        //AddMessage2Log(print_r(array('$src' => $src), true));
        if (!$strict) {
            return true;
        }
    ...
    public function parseCaralogFilterSrc($src) {
        ...
        if ($countPoint >= 2 && preg_match("/^\/{2}/", $src) && !preg_match("/^\/{2}www\./", $src) && !preg_match("/http:\//", $src))
            $src = preg_replace("/^\/{2}/", "http://", $src);
        //$src = str_replace('http:/', 'http://', $src); 
        $src = str_replace('//', '/', $src);
        $src = str_replace('http:/', 'http://', $src);
        if (preg_match("/www\./", $src) || preg_match("/http:\//", $src))
            $src = preg_replace("/^\/{2}/", "http://", $src);
        if (preg_match("/www\./", $src) || preg_match("/http:\//", $src))
    ...
    protected function parseCatalogNavigation($pageHref) {
        ...
        if ($this->settings["catalog"]["pagenavigation_selector"]) {
            ...
            foreach ($this->html[$element] as $page) {
                ...
                if (isset($this->pagenavigationPrev[$p]))
                    continue;

                if ($this->IsNumberPageNavigation()) {
                    if (!$this->CheckValidatePageNavigation($n) && !$this->CheckPageNavigation($n))
                        continue;

                    if (($this->currentPage + 5) < $n)
                        continue;
                    if ($this->CheckPageNavigation($n)) {
                        if (isset($this->pagenavigationPrev[$p]))
                            continue;
                        if (isset($this->pagenavigationPrev[$n]))
                            continue;
    ...
    public function changeImgSrc($html) {
        //AddMessage2Log(print_r(array('$html' => $html), true));
        phpQuery::selectDocument($html);
    ...
    public function parserFirstImg($html) {
        ...
        $site = $this->getUrlSite();
        foreach ($html["img"] as $img) {
           // AddMessage2Log(print_r(array('$img' => $img), true));
    ...
    public function filterSrc($src) {
        ...
        if (preg_match("/www\./", $src) || preg_match("/http:\//", $src))
            $src = preg_replace("/^\/{1}/", "http://", $src);
        //$src = str_replace('//', '/', $src);
        //AddMessage2Log(print_r(array('$src_' => $src), true));
        return $src;
    }
